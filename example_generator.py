...
class ExampleGenerator:
    def __init__(self):
        ...
        self.gen_tokenizer = MLMTokenizer.from_pretrained(
            "laihuiyuan/DRS-LMM", src_lang="<drs>"
        )
    ...
    def drs2en(self, gold_drs: str) -> str:
        ...
        forced_ids = self.gen_tokenizer.encode(
            "en_XX", add_special_tokens=False, return_tensors="pt"
        )
        outs = self.model.generate(
            input_ids=inp_ids,
            forced_bos_token_id=forced_ids.item(),
            num_beams=5,
            max_length=150,
        )
    ...
    def en2drs(self, text: str) -> str:
        ...
        forced_ids = self.parse_tokenizer.encode(
            "<drs>", add_special_tokens=False, return_tensors="pt"
        )
        outs = self.model.generate(
            input_ids=inp_ids,
            forced_bos_token_id=forced_ids.item(),
        ...
        drs = self.parse_tokenizer.decode(
            outs[0].tolist(),
            skip_special_tokens=True,
            clean_up_tokenization_spaces=False,
        )
